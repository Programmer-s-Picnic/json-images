(function () {
  ...
  const PPSpeakDefaults = {
    ...
    controlsContainerId: "pp-auto-speak-controls-v8-2",
    ...
    storageKeyPosition: "ppSpeakPanelPositionV82",
    storageKeyMiniMode: "ppSpeakMiniModeV82",
    storageKeyRate: "ppSpeakRateV82",
    storageKeyPitch: "ppSpeakPitchV82",
    storageKeyVoice: "ppSpeakVoiceV82"
  };
  ...
  class UI {
    ...
    injectStyle() {
      ...
      style.textContent = `
        ...
        .pp-speak-panel{
          position:fixed;right:14px;bottom:14px;width:min(396px,calc(100vw - 20px));z-index:99999;
        ...
        .pp-speak-head{
          padding:14px 14px 10px;background:
          ...
          border-bottom:1px solid rgba(217,119,6,.10);cursor:grab;touch-action:none;
        }
        ...
        .pp-speak-avatar-shell{
          display:grid;grid-template-columns:108px 1fr;gap:16px;align-items:center;background:rgba(255,255,255,.9);
          border:1px solid rgba(217,119,6,.10);border-radius:22px;padding:16px;
          box-shadow:0 10px 30px rgba(0,0,0,.05);
        }
        .pp-speak-avatar-wrap{position:relative;width:108px;height:108px;display:grid;place-items:center;flex-shrink:0;}
        .pp-speak-avatar-ring{
          position:absolute;inset:-7px;border-radius:50%;
          background:radial-gradient(circle, rgba(245,158,11,.24), rgba(245,158,11,.05) 58%, transparent 76%);
          opacity:.7;
        }
        .pp-speak-panel.is-speaking .pp-speak-avatar-ring{animation:ppAvatarGlow 1.2s ease-in-out infinite;}
        @keyframes ppAvatarGlow{0%{transform:scale(1)}50%{transform:scale(1.1)}100%{transform:scale(1)}}
        .pp-speak-avatar{
          position:relative;width:96px;height:96px;border-radius:50%;overflow:hidden;
          border:4px solid #fff;box-shadow:0 8px 24px rgba(0,0,0,.15), 0 0 0 6px rgba(217,119,6,.12);background:#ead7c4;
          transition:transform .24s ease;
        }
        .pp-speak-panel.is-speaking .pp-speak-avatar{animation:ppAvatarFloat 3s ease-in-out infinite;}
        @keyframes ppAvatarFloat{0%{transform:translateY(0)}50%{transform:translateY(-4px)}100%{transform:translateY(0)}}
        .pp-speak-avatar-img{width:100%;height:100%;object-fit:cover;display:block;filter:saturate(1.02) contrast(1.02);}
        .pp-speak-avatar-facefx{position:absolute;inset:0;pointer-events:none;}
        .pp-speak-eye-blink{
          position:absolute;top:35px;width:13px;height:2px;background:rgba(20,20,20,.76);border-radius:999px;opacity:0;transform:scaleY(0.15);
        }
        .pp-speak-eye-blink.left{left:25px}.pp-speak-eye-blink.right{right:25px}
        .pp-speak-avatar.is-image.pp-blink .pp-speak-eye-blink{animation:ppImageBlink 5s infinite;}
        @keyframes ppImageBlink{0%,44%,48%,100%{opacity:0;transform:scaleY(0.15)}45%,47%{opacity:.95;transform:scaleY(1)}}
        .pp-speak-mouth-image{
          position:absolute;left:50%;top:63%;transform:translateX(-50%);
          width:24px;height:6px;border-radius:0 0 12px 12px;background:rgba(110,35,20,.88);
          box-shadow:0 0 0 1px rgba(255,255,255,.14) inset;opacity:.92;
        }
        .pp-speak-mouth-image::after{
          content:"";position:absolute;left:50%;bottom:-1px;transform:translateX(-50%);
          width:12px;height:5px;border-radius:10px 10px 0 0;background:rgba(236,72,153,.62);
        }
        .pp-speak-avatar-mouth-shape{
          position:absolute;left:50%;top:63%;transform:translateX(-50%);
          width:24px;height:6px;border-radius:999px;opacity:0;
        }
        .pp-speak-panel.is-speaking .pp-speak-avatar.is-image .pp-speak-mouth-shape.shape-a{animation:ppShapeA .42s linear infinite}
        .pp-speak-panel.is-speaking .pp-speak-avatar.is-image .pp-speak-mouth-shape.shape-b{animation:ppShapeB .42s linear infinite}
        .pp-speak-panel.is-speaking .pp-speak-avatar.is-image .pp-speak-mouth-shape.shape-c{animation:ppShapeC .42s linear infinite}
        ...
        .pp-speak-panel.is-speaking .pp-speak-avatar.is-image .pp-speak-mouth-image{animation:ppLipShadow 0.16s ease-in-out infinite alternate;}
        .pp-speak-panel.is-paused .pp-speak-avatar.is-image .pp-speak-mouth-image{animation:none;height:5px;top:64%;}
        @keyframes ppLipShadow{0%{opacity:.75}100%{opacity:0}}

        .pp-speak-avatar.pp-default-avatar .pp-speak-hair{position:absolute;left:7px;right:7px;top:6px;height:26px;background:linear-gradient(180deg,#27272a,#3f3f46);border-radius:26px 26px 12px 12px;}
        .pp-speak-avatar.pp-default-avatar .pp-speak-face{position:absolute;inset:16px 10px 8px 10px;border-radius:24px 24px 30px 30px;background:linear-gradient(180deg,#ffd7ae,#f3ba84);}
        .pp-speak-avatar.pp-default-avatar .pp-speak-eye{position:absolute;top:23px;width:9px;height:6px;background:#1f2937;border-radius:0 0 10px 10px;animation:ppAvatarBlink 5s infinite;transform-origin:center center;}
        .pp-speak-avatar.pp-default-avatar .pp-speak-eye.left{left:14px}.pp-speak-avatar.pp-default-avatar .pp-speak-eye.right{right:14px}
        @keyframes ppAvatarBlink{0%,45%,47%,100%{transform:scaleY(1)}46%{transform:scaleY(.08)}}
        .pp-speak-avatar.pp-default-avatar .pp-speak-mouth{position:absolute;left:50%;top:38px;width:18px;height:5px;transform:translateX(-50%);background:#7c2d12;border-radius:0 0 10px 10px;overflow:hidden;}
        .pp-speak-avatar.pp-default-avatar .pp-speak-mouth::after{content:"";position:absolute;left:50%;bottom:-1px;width:10px;height:5px;transform:translateX(-50%);background:#ef9aa8;border-radius:10px 10px 0 0;opacity:.9;}
        .pp-speak-panel.is-speaking .pp-speak-avatar.pp-default-avatar .pp-speak-mouth{animation:ppTalkMouth .16s ease-in-out infinite alternate;}
        .pp-speak-panel.is-paused .pp-speak-avatar.pp-default-avatar .pp-speak-mouth{animation:none;height:4px;top:39px;}
        ...
        .pp-speak-panel.is-speaking .pp-speak-wave span{animation:ppWaveBounceV82 1s ease-in-out infinite;}
        ...
        @keyframes ppWaveBounceV82{0%,100%{height:6px}50%{height:20px}}
        ...
        .pp-speak-select,.pp-speak-btn{border-radius:12px;border:1px solid rgba(217,119,6,.14);background:rgba(255,255,255,.96);color:#172033;font-size:13px;padding:10px 11px;width:100%;box-sizing:border-box;outline:none;}
        ...
        .pp-speak-btn.pp-whatsapp-btn{position:relative;overflow:hidden;background:linear-gradient(135deg,#25D366,#128C7E);color:#ffffff;box-shadow:0 14px 30px rgba(18,140,126,.28),0 8px 18px rgba(37,211,102,.18);animation:ppWhatsappFloat 2.7s ease-in-out infinite;}
        ...
        .pp-speak-meta{display:grid;gap:7px;background:rgba(255,255,255,.82);border:1px solid rgba(217,119,6,.10);border-radius:14px;padding:10px 11px;}
        ...
        @media (max-width:640px){
          ...
          .pp-speak-avatar-shell{grid-template-columns:76px 1fr;gap:12px;padding:12px;border-radius:18px;align-items:center;}
          .pp-speak-avatar-wrap{width:76px;height:76px}
          .pp-speak-avatar{width:70px;height:70px;box-shadow:0 6px 20px rgba(0,0,0,.14),0 0 0 5px rgba(217,119,6,.10)}
          .pp-speak-eye-blink{top:26px;width:10px}
          .pp-speak-eye-blink.left{left:18px}.pp-speak-eye-blink.right{right:18px}
          .pp-speak-mouth-image,.pp-speak-avatar-mouth-shape{width:18px}
          .pp-speak-avatar-name{font-size:17px;line-height:1.1}
          .pp-speak-avatar-role{font-size:12px}
          .pp-speak-avatar-caption{margin-top:6px;font-size:12px;min-height:24px}
          .pp-speak-wave{margin-top:6px;height:18px}
    ...
    renderPanel() {
      ...
      const useImage = !!(opts.avatarUseImage && opts.avatarImage);
      const avatarMarkup = useImage
        ? `<img class="pp-speak-avatar-img" src="${U.escapeHtml(opts.avatarImage)}" alt="${U.escapeHtml(opts.avatarImageAlt)}">
      ...
      const avatarClass = useImage ? "is-image pp-blink" : "pp-default-avatar";
      ...
      panel.innerHTML = `
        <div class="pp-speak-head" id="pp-speak-drag-handle">
          <div class="pp-speak-topline">
            <div>
              <div class="pp-speak-kicker">Auto Narration V8.2</div>
        ...
        <div class="pp-speak-body">
          <div class="pp-speak-avatar-shell">
            <div class="pp-speak-avatar-wrap">
              ...
              <div class="pp-speak-avatar ${avatarClass}">
                ${avatarMarkup}
              </div>
            </div>
    ...
    activateAvatarSpeaking() { const panel = this.panel; if (panel) { panel.classList.add("is-speaking"); panel.classList.remove("is-paused"); } }
    activateAvatarPaused() { const panel = this.panel; if (panel) { panel.classList.remove("is-speaking"); panel.classList.add("is-paused"); } }
    activateAvatarIdle() { const panel = this.panel; if (panel) panel.classList.remove("is-speaking", "is-paused"); }
  ...
  class Engine {
    constructor(userOptions) {
      ...
      this.items = [];
      this.state = { currentIndex: 0, running: false, paused: false, speaking: false, activeUtterance: null, dragged: false, speechToken: 0 };
      this.ui = new UI(this);
    ...
    log(message) { if (this.options.log) console.log("[PPSpeakV82]", message); }

    init(userOptions) {
      if (userOptions && typeof userOptions === "object") this.options = Object.assign({}, this.options, userOptions);
      if (this._initialized) { this.refresh(); this.loadVoices(); this.ui.updateUI(); return this; }
      ...
      if ("speechSynthesis" in window) {
        ...
        if (typeof window.speechSynthesis.addEventListener === "function") window.speechSynthesis.addEventListener("voiceschanged", this.boundVoicesChanged);
        else window.speechSynthesis.onvoiceschanged = this.boundVoicesChanged;
      }
    ...
    collectItems() {
      ...
      this.items = found.map((el, index) => ({ index, id: el.id, el, text: this.getSpeakText(el), scrollTarget: this.getScrollTarget(el) }));
    }
    ...
    scrollToItem(item) {
      ...
      try { item.scrollTarget.scrollIntoView({ behavior: this.options.scrollBehavior, block: this.options.scrollBlock }); } catch {}
    }
    ...
    resume() {
      if (!this.state.running || !this.state.paused) return;
      this.state.paused = false;
      if ("speechSynthesis" in window) { try { speechSynthesis.resume(); } catch { this.speakCurrent(); return; } }
    ...
    destroy() {
      ...
      this.ui.destroy();
      this.plugins.clear();
      delete window.PPSpeakV82;
      delete window.PPSpeakV8;
      delete window.PPSpeakV7;
    ...
    bindGlobalAPI() {
      ...
      window.PPSpeakV82 = api;
      window.PPSpeakV8 = api;
      window.PPSpeakV7 = api;
  ...
  function boot() {
    const existing = window.__PPSPEAK_V82_INSTANCE__;
    if (existing && typeof existing.destroy === "function") existing.destroy();
    const app = new Engine();
    window.__PPSPEAK_V82_INSTANCE__ = app;
    app.init();
  }

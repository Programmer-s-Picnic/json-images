(function () {
  ...
  const PPSpeakDefaults = {
    ...
    controlsContainerId: "pp-auto-speak-controls-v6-opt",
    ...
    storageKeyPosition: "ppSpeakPanelPositionV6Opt",
    storageKeyMiniMode: "ppSpeakMiniModeV6Opt",
    storageKeyRate: "ppSpeakRateV6Opt",
    storageKeyPitch: "ppSpeakPitchV6Opt",
    storageKeyVoice: "ppSpeakVoiceV6Opt"
  };
  ...
  class PPSpeakUI {
    ...
    get styleId() {
      return this.engine.options.controlsContainerId + "-style";
    }
    ...
    renderPanel() {
      ...
      const whatsappBodyRow = opts.whatsappEnabled ? `
        <div class="pp-speak-row" data-plugin="whatsapp-body">
          ...
          >${PPSpeakUtils.escapeHtml(opts.whatsappLabel)}</button>
        </div>` : "";
      ...
      panel.innerHTML = `
        <div class="pp-speak-head" id="pp-speak-drag-handle">
          <div class="pp-speak-topline">
            <div>
              <div class="pp-speak-kicker">Auto Narration V6 Optimized</div>
    ...
    bindPanelEvents() {
      ...
      if (whatsappBtn) this.on(whatsappBtn, "click", () => this.engine.openWhatsApp());

      const whatsappMiniBtn = panel.querySelector("#pp-speak-mini-whatsapp");
      if (whatsappMiniBtn) this.on(whatsappMiniBtn, "click", () => this.engine.openWhatsApp());
    ...
    updateVoiceOptions() {
      ...
      select.innerHTML = `<option value="">Default</option>` + voices.map((v) => {
        ...
        const label = PPSpeakUtils.escapeHtml(v.name + (v.lang ? " (" + v.lang + ")" : ""));
    ...
    restoreMiniMode() {
      ...
      const raw = PPSpeakUtils.storageGet(this.engine.options.storageKeyMiniMode);
  ...
  class PPSpeakCore {
    ...
    log(message) {
      if (this.options.log) console.log("[PPSpeakV6Optimized]", message);
    }
    ...
    highlight(item) {
      ...
      if (item.el && !PPSpeakUtils.isHidden(item.el)) item.el.classList.add(this.options.activeClass);
      if (item.scrollTarget && item.scrollTarget !== item.el && !PPSpeakUtils.isHidden(item.scrollTarget)) {
    ...
    pause() {
      ...
      if ("speechSynthesis" in window) {
        try { speechSynthesis.pause(); } catch (err) { this.log("Pause not supported cleanly."); }
      }
    ...
    speakCurrent() {
      ...
      try {
        ...
        this.log("Speech synthesis speak failed.");
        this.state.currentIndex += 1;
        this.speakCurrent();
      }
    }
    ...
    savePanelPosition() {
      ...
      PPSpeakUtils.storageSet(this.options.storageKeyPosition, JSON.stringify({ left: rect.left, top: rect.top }));
    }
    ...
    restorePanelPosition() {
      ...
      const raw = PPSpeakUtils.storageGet(this.options.storageKeyPosition);
      if (!raw) return;

      const data = PPSpeakUtils.safeJsonParse(raw);
    ...
    resetPanelPosition() {
      ...
      PPSpeakUtils.storageRemove(this.options.storageKeyPosition);
    }

    saveMiniMode(isMini) {
      PPSpeakUtils.storageSet(this.options.storageKeyMiniMode, isMini ? "1" : "0");
    }
    ...
    restorePreferences() {
      const savedRate = PPSpeakUtils.storageGet(this.options.storageKeyRate);
      const savedPitch = PPSpeakUtils.storageGet(this.options.storageKeyPitch);
      const savedVoice = PPSpeakUtils.storageGet(this.options.storageKeyVoice);
    ...
    savePreferences() {
      PPSpeakUtils.storageSet(this.options.storageKeyRate, String(this.options.rate));
      PPSpeakUtils.storageSet(this.options.storageKeyPitch, String(this.options.pitch));
      PPSpeakUtils.storageSet(this.options.storageKeyVoice, String(this.options.voiceName || ""));
    }

    enableDragging(panel, handle) {
      if (!panel || !handle || this._dragCleanup) return;
    ...
    destroy() {
      ...
      if (this.boundVoicesChanged && "speechSynthesis" in window) {
        ...
        } else if (window.speechSynthesis.onvoiceschanged === this.boundVoicesChanged) {
  ...
  class PPSpeakTitleButtonPlugin {
    afterInit(engine) {
      if (engine.options.addTitleButton) engine.ui.attachTitleButton();
    }
  }

  function boot() {
    const existing = window.__PPSPEAK_V6_OPT_INSTANCE__;
    if (existing && typeof existing.destroy === "function") existing.destroy();

    const app = new PPSpeakCore();
    window.__PPSPEAK_V6_OPT_INSTANCE__ = app;
    app.init();
  }
